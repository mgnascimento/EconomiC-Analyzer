...
class regionDAO
{
    ...
    public function remover ($region)
    {
        ...
        try {
            ...
            $statement = $pdo->prepare("DELETE FROM tb_region WHERE id_region = :id_region");
    ...
    public function atualizar($region){
        global $pdo;
        try {
            $statement = $pdo->prepare("SELECT id_region, str_name_region FROM tb_region WHERE id_region = :id_region");
            $statement->bindValue(":id_region", $region->getIdRegion());
            if ($statement->execute()) {
                $rs = $statement->fetch(PDO::FETCH_OBJ);
                $region->setIdRegion($rs->id_region);
                $region>setNameRegion($rs->name_region);
    ...
    public function tabelapaginada() {
     ...
     <tbody>";
            foreach($dados as $inst):
                echo "<tr>
        <td>$inst->id_region</td>
        <td>$inst->name_region</td>
        <td><a href='?act=upd&id=$inst->id_region'><i class='ti-reload'></i></a></td>
        <td><a href='?act=del&id=$inst->id_region'><i class='ti-close'></i></a></td>
       </tr>";
            endforeach;
            echo"
</tbody>
