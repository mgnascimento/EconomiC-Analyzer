...
class beneficiariesDAO
{
    ...
    public function salvar($bene)
    {
        ...
        try {
            ...
            if ($bene->getIdBeneficiaries() != "") {
                ...
                $statement->bindValue(":id_action", $bene->getIdBeneficiaries());
            } else {
    ...
    public function atualizar($bene){
        global $pdo;
        try {
            $statement = $pdo->prepare("SELECT id_beneficiaries, str_nis, str_name_beneficiaries FROM tb_beneficiaries WHERE id_beneficiaries = :id_beneficiaries");
            $statement->bindValue(":id", $bene->getIdBeneficiaries());
            if ($statement->execute()) {
                $rs = $statement->fetch(PDO::FETCH_OBJ);
                $bene->setIdBeneficiaries($rs->id_beneficiaries);
                $bene>setNis($rs->nis);
                $bene>setNamePerson($rs->name_person);
