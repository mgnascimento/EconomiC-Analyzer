...
class actionDAO
{
    public function remover ($action)
    {
        ...
        try {
            ...
            $statement = $pdo->prepare("DELETE FROM tb_action WHERE id_action = :id_action");
    ...
    public function salvar($action)
    {
        ...
        try {
            ...
            if ($action->getIdAction() != "") {
                $statement = $pdo->prepare("UPDATE tb_action SET str_cod_action=:cod_action, str_name_action=:name_action WHERE id_action = :id_action;");
                $statement->bindValue(":id_action", $action->getIdAction());
            } else {
                $statement = $pdo->prepare("INSERT INTO tb_action (str_cod_action, str_name_action) VALUES (:cod_action, :name_action)");
            }
            $statement->bindValue(":cod_action", $action->getCodAction());
            $statement->bindValue(":name_action", $action->getNameAction());
