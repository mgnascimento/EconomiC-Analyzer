...
class programDAO
{
    ...
    public function remover ($program)
    {
        ...
        try {
            ...
            $statement = $pdo->prepare("DELETE FROM tb_program WHERE id_program = :id_program");
            $statement->bindValue(":id_program", $program->getIdProgram());
    ...
    public function salvar($program)
    {
        ...
        try {
            ...
            if ($program->getIdProgram() != "") {
                $statement = $pdo->prepare("UPDATE tb_program SET str_cod_program=:cod_program, str_name_program=:name_program WHERE id_program = :id_program");
                $statement->bindValue(":id_program", $program->getIdProgram());
            } else {
                $statement = $pdo->prepare("INSERT INTO tb_program (str_cod_program, str_name_program) VALUES (:cod_program, :name_program )");
            }
            $statement->bindValue(":cod_program", $program->getCodProgram());
            $statement->bindValue(":name_program", $program->getNameProgram());
    ...
    public function atualizar($program){
        global $pdo;
        try {
            $statement = $pdo->prepare("SELECT id_program, str_cod_program, str_name_program FROM tb_program WHERE id_program = :id_program");
            $statement->bindValue(":id_program", $program->getIdProgram());
            if ($statement->execute()) {
                $rs = $statement->fetch(PDO::FETCH_OBJ);
                $program->setIdProgram($rs->id_program);
                $program->setCodProgram($rs->str_cod_program);
    ...
    public function tabelapaginada() {
     ...
     <tbody>";
        ...
        <td><a href='?act=upd&id_program=$inst->id_program'><i class='ti-reload'></i></a></td>
        <td><a href='?act=del&id_program=$inst->id_program'><i class='ti-close'></i></a></td>
       </tr>";
            endforeach;
            echo"
</tbody>
